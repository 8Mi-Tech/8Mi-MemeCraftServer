        ...
        public void accept(ChannelHandlerContext ctx, FMLHandshakeMessage msg, Consumer<? super FMLHandshakeServerState> cons)
        {
            ...
            if (!ctx.channel().attr(NetworkDispatcher.IS_LOCAL).get())
            {
                ...
                while (itr.hasNext())
                {
                    ...
                    if ("minecraft:dataserializers".equals(e.getKey().toString()) && !CumServer.isSendDataSerializers(ctx.channel().attr(NetworkDispatcher.FML_DISPATCHER).get().getModList())) continue; // CatServe
